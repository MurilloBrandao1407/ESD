...
void calcular_estatisticas(Nodo* lista) {
    ...
    printf("Insolacao   - min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min1, s1/cont, max1, sqrt(s1q/cont - pow(s1/cont, 2)));
    printf("Precipitacao- min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min2, s2/cont, max2, sqrt(s2q/cont - pow(s2/cont, 2)));
    printf("Temp Max    - min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min3, s3/cont, max3, sqrt(s3q/cont - pow(s3/cont, 2)));
    printf("Temp Min    - min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min4, s4/cont, max4, sqrt(s4q/cont - pow(s4/cont, 2)));
    printf("Umidade     - min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min5, s5/cont, max5, sqrt(s5q/cont - pow(s5/cont, 2)));
    printf("Vento       - min: %.2f, media: %.2f, max: %.2f, desvio padrao: %.2f\n", min6, s6/cont, max6, sqrt(s6q/cont - pow(s6/cont, 2)));
}
...
int main() {
    ...
    printf("Tempo medio por busca: %.6f segundos\n", tempo_medio);
            ...
            if (total_resultados > 0) {
               ...
               if (strcmp(resposta, "sim") == 0 || strcmp(resposta, "SIM") == 0) {
    			clock_t inicio = clock();
    			lista = remover_resultados(lista);
    			clock_t fim = clock();
   				 printf("Tempo para remocao em massa: %.6f segundos\n", (double)(fim - inicio) / CLOCKS_PER_SEC);
    ...
    printf("Todos os dados foram apagados.\n");
    		...
    		clock_t fim = clock();
    		printf("Tempo para calcular estatisticas: %.6f segundos\n", (double)(fim - inicio) / CLOCKS_PER_SEC);
		}			
 else if (opcao == 0) {
