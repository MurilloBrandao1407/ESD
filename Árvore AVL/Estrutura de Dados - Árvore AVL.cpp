...
void mostrar_estatistica(const char* nome, float soma, float soma2, float min, float max, int cont) {
    ...
    printf("%s: min: %.2f ; media: %.2f ; max: %.2f ; desvio padrao: %.2f\n",
...
if (total_resultados > 0) {
    ...
    if (raiz == NULL) {
        ...
        mostrar_estatistica("Insolacao",    s1,  s1q, min1, max1, cont);
        mostrar_estatistica("Precipitacao", s2,  s2q, min2, max2, cont);
        mostrar_estatistica("Temp Max",     s3,  s3q, min3, max3, cont);
        mostrar_estatistica("Temp Min",     s4,  s4q, min4, max4, cont);
        mostrar_estatistica("Umidade",      s5,  s5q, min5, max5, cont);
        mostrar_estatistica("Vento",        s6,  s6q, min6, max6, cont);
